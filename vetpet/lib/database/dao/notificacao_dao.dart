...
class NotificacaoDao extends ChangeNotifier{
  ...
  static const String tableSql = 'CREATE TABLE IF NOT EXISTS notificacoes ('
  ...
  Future<int> updateNotificacaoAviso(Notificacao notificacao, int id) async {
    try{
      ...
      if( await db.update( tablename, notificacaoMap, where: '$_idaviso = ?', whereArgs: [id]) >= 1){
        return 1;
      }else{
  ...
  static verificaNotificacao( ) async {
    final String dtBuscaAviso = DateTime.now().year.toString() + DateTime.now().month.toString().padLeft(2,'0') +(DateTime.now().day+5).toString().padLeft(2,'0')   ;
    final Database db = await getDatabase();
    ...
    for (Map<String, dynamic> row in resultnot) {
      developer.log('verificaNotificacao for $row[_idaviso]');
      if(row[_idaviso] > 0) {
