...
class AvisoDao extends ChangeNotifier{
  ...
  static const String tableSql = 'CREATE TABLE IF NOT EXISTS avisos ('
  ...
  static const String tablename = "avisos";
  ...
  Future<List<Aviso>> findVacinasVencendo() async {
    ...
    throw('AvisoDao findVacinasVencendo ');
    }
  }
  ...
  Future<int> updateAviso(Aviso aviso, int idaviso,[String statusNotificacoa= "A"]) async {
   try{
      final Database db = await getDatabase();
      Map<String, dynamic> avisoMap = _toMap(aviso);
      final Notificacao notificacao =  Notificacao(0,0,aviso.id,aviso.idpet ,aviso.datacadastro,"",aviso.datavencimento,statusNotificacoa);
      notificacao.calculaInicio();
  ...
  Future<int> deleteAviso( int idaviso) async {
    try{
      ...
      return db.delete(
        ...
        return true ;//notificacaoDao.deletenotificacaoAviso(idaviso);
      });
    }
    catch(e, s){
  ...
  static verificaAvisoVencendo( int id, int idnotivicacao) async {
    ...
    try{
      ...
      for (Map<String, dynamic> row in result) {
        ...
        //developer.log('verificaAvisoVencendo  ${row}');
