...
import 'package:vetpet/model/Opcoes.dart';
...
class Vacina_Dao extends ChangeNotifier{
  ...
  Future<List<Opcoes>> opcoesVacinas() async {
    final Database db = await getDatabase();
    final List<Map<String, dynamic>> result = await db.rawQuery("SELECT distinct $_nome_vacina vacina from $tablename" );
    List<Opcoes> _Options = [];
    for (Map<String, dynamic> row in result) {
      developer.log(row['vacina'].toString());
      Opcoes op = Opcoes(descOpcoes: row['vacina'].toString() );
      _Options.add(op);
    }
    return    _Options ;
  }
